...
MutableSelectable implements the [SelectableInterface](./selectable-interface) and provides some additional helper functions like [`.flush`](#delay-updating-listeners-while-making-multiple-changes-to-state)
...
This is incremented every time the state is changed.
...
Delays calling listeners until the callback has finished executing.
This is useful when you want to make multiple changes to state and only want to the listeners to update once.
