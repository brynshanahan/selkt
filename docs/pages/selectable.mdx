...
Selectable extends [MutableSelectable](./mutable-selectable) and implements the [SelectableInterface](./selectable-interface). The only difference between this and MutableSelectable is that this uses [`immer`](https://github.com/immerjs/immer) to ensure that mutations in `.set` calls are immutable
